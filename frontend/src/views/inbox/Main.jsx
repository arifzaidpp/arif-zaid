...
//                       >
//                         <Lucide
//                           icon={message.isStared ? "Star" : "Star"}
//                           className={`w-4 h-4 ${message.isStared
//                             ? "text-yellow-500"
//                             : "text-slate-400"
//                             }`}
//                         />
...
function Main() {
  ...
  const { contacts, loading, error, refetch } = useGetAllContacts();
  const { deleteContact, loadingDelete, errorDelete } = useDeleteContact();
  const { updateContact, loadingUpdate, errorUpdate } = useUpdateContact();

  useEffect(() => {
    generateMessages(contacts);
  }, [contacts]);

  const generateMessages = (contacts) => {
    if (contacts) {
      const newGeneratedMessages = contacts.map((contact) => ({
        id: contact._id,
        senderName: `${contact.firstName} ${contact.lastName}`,
        senderPhoto: contact.photo || `https://avatar.iran.liara.run/public/boy?username=${contact.email}`,
        senderEmail: contact.email || "Not Given",
        senderPhone: contact.phone || "Not Given",
        subject: contact.subject || "No Subject",
        body: contact.message || "No body content available",
        time: (() => {
          ...
          return isToday
            ...
            : messageDate.toLocaleDateString("en-US", { year: "numeric", month: "2-digit", day: "2-digit" });
        })(),
        date: new Date(contact.createdAt).toLocaleDateString("en-US"),
        isRead: contact.isRead || false,
        isSelected: false,
        isStared: contact.isStared || false,
      }));
      setMessages(newGeneratedMessages);
    }
  };

  const handleReload = async () => {
    await refetch(); // Trigger a refetch of contacts
    generateMessages(contacts); // Re-generate messages from the newly fetched contacts
  };
  ...
  const handleDelete = async () => {
    ...
    if (selectedMessage) {
      ...
      selectedMessages = messages.filter((message) => message.isSelected);
    }

    for (const message of selectedMessages) {
      await deleteContact(message.id);
      handleBackToList(); // Reset selected message
    }

    // Optionally, you can reload the messages after deletion
    handleReload();
  };
  ...
  const handleStarMessage = async (id) => {
    setMessages((prevMessages) =>
      prevMessages.map((message) =>
        message.id === id
          ? { ...message, isStared: message.isRead ? !message.isStared : message.isStared }
          : message
      )
    );

    if (selectedMessage && selectedMessage.id === id && selectedMessage.isRead) {
      setSelectedMessage((prevMessage) => ({
        ...prevMessage,
        isStared: !prevMessage.isStared,
      }));
    }

    const messageToUpdate = messages.find((message) => message.id === id);

    if (messageToUpdate && messageToUpdate.isRead) {
      await updateContact(id, { isStared: !messageToUpdate.isStared });
    }
  };
  ...
  const handleShowDetails = async (message) => {
    setSelectedMessage(message);
    setCurrentIndex(filteredMessages.findIndex(m => m.id === message.id));
    //update message as read
    if (!message.isRead) {
      await updateContact(message.id, { isRead: true });
    }
  };
  ...
  const handleBackToList = () => {
    setSelectedMessage(null);
    setCurrentIndex(0);
    handleReload(); // Reload messages after deletion
    setSelectedMessage(null);
  };
  ...
  const showPreviousMessage = async () => {
    ...
    if (!newMessage.isRead) {
      await updateContact(newMessage.id, { isRead: true });
    }
  };

  const showNextMessage = async () => {
    const newIndex = currentIndex < filteredMessages.length - 1 ? currentIndex + 1 : filteredMessages.length - 1;
    const newMessage = filteredMessages[newIndex];
    setCurrentIndex(newIndex);
    setSelectedMessage(newMessage);
    if (!newMessage.isRead) {
  ...
  // Render messages (this part is omitted, but you would include the rendering of messages here)
        ...
        <div className="col-span-12 lg:col-span-3 2xl:col-span-2">
          ...
          <div className="intro-y box bg-primary p-5 mt-6">
            ...
            <div className="border-t border-white/10 dark:border-darkmode-400 mt-6 pt-6 text-white">
              <div
                onClick={handleInbox}
                className={`flex items-center px-3 py-2 rounded-md ${selectedMenu === 'Inbox' ? 'change bg-white/10 dark:bg-darkmode-700 font-medium' : ''
                  }`}
              >
                <Lucide icon="Mail" className="w-4 h-4 mr-2" /> Inbox
              </div>
              <div
                ...
                className={`flex items-center px-3 py-2 mt-2 rounded-md ${selectedMenu === 'Marked' ? 'change bg-white/10 dark:bg-darkmode-700 font-medium' : ''
                  }`}
              >
                <Lucide icon="Star" className="w-4 h-4 mr-2" /> Marked
              </div>
              <div
                onClick={() => setSelectedMenu('Draft')}
                className={`flex items-center px-3 py-2 mt-2 rounded-md ${selectedMenu === 'Draft' ? 'change bg-white/10 dark:bg-darkmode-700 font-medium' : ''
                  }`}
              >
                <Lucide icon="Inbox" className="w-4 h-4 mr-2" /> Draft
              </div>
              <div
                onClick={() => setSelectedMenu('Sent')}
                className={`flex items-center px-3 py-2 mt-2 rounded-md ${selectedMenu === 'Sent' ? 'change bg-white/10 dark:bg-darkmode-700 font-medium' : ''
                  }`}
              >
                <Lucide icon="Send" className="w-4 h-4 mr-2" /> Sent
              </div>
              <div
                ...
                className={`flex items-center px-3 py-2 mt-2 rounded-md ${selectedMenu === 'Trash' ? 'change bg-white/10 dark:bg-darkmode-700 font-medium' : ''
                  }`}
              >
                <Lucide icon="Trash" className="w-4 h-4 mr-2" /> Trash
              </div>
              ...
              <div className="flex-1 px-2">
                <div className="h-16 flex items-center justify-between">
                  <div className="flex items-center">
                    ...
                    <div className="flex items-center">
                      ...
                      <div className="flex items-center space-x-2">
                        ...
                        <button onClick={handleDelete} title="Delete" className=" px-2 py-1 border border-gray-300 rounded-lg shadow hover:bg-gray-200 dark:hover:bg-gray-700 transition duration-100">
                          <Lucide icon="Trash" className="h-5 w-5" />
                        </button>
                      </div>
                      <span className="bg-gray-300 h-6 w-[.5px] mx-3"></span>
                      <div className="flex items-center space-x-2">
                        ...
                        >
                          {selectedMessage.isStared ? <FaStar className="w-4 h-4 text-yellow-500" /> : <FaRegStar className="w-4 h-4" />}
                        </button>
                      </div>
                    </div>
                  </div>
                  <div className="px-2 flex items-center space-x-4">
                    <div className="flex items-center space-x-2">
                      <button
                        className={`bg-gray-200 hover:bg-gray-300 text-gray-700 p-1.5 rounded-lg transition duration-150 ${currentIndex === 0 ? "opacity-50 cursor-not-allowed" : ""}`}
                        title="Previous Email"
                        onClick={showPreviousMessage}
                      ...
                      <button
                        className={`bg-gray-200 hover:bg-gray-300 text-gray-700 p-1.5 rounded-lg transition duration-150 ${currentIndex === filteredMessages.length - 1 ? "opacity-50 cursor-not-allowed" : ""}`}
                        title="Next Email"
                        onClick={showNextMessage}
                ...
                <div className="m-6">
                  ...
                  <div className="flex items-center justify-between">
                    <div className="flex items-center">
                      <img src={selectedMessage.senderPhoto} alt="Sender" className="rounded-full w-8 h-8 border border-gray-500" />
                      <div className="flex flex-col ml-2">
                        <span className="text-sm font-semibold">{selectedMessage.senderName}</span>
                        <span className="text-xs text-gray-400">From: {selectedMessage.senderEmail} <br /> Phone: {selectedMessage.senderPhone}</span>
                      </div>
                    </div>
                    <span className="text-sm">{selectedMessage.date} <br /> {selectedMessage.time}</span>
                  </div>
                  <div className="py-6 pl-2">{selectedMessage.body}</div>
                  {/* Reply and Forward buttons */}
                  <div className="mt-8 pb-6 flex items-center space-x-4">
                    <button className="w-32 flex items-center justify-center space-x-2 py-1.5 border border-gray-400 rounded-lg hover:bg-gray-200" aria-label="Reply">
                      <Lucide icon="Reply" className="h-5 w-5" />
                      <span>Reply</span>
                    </button>
                    <button className="w-32 flex items-center justify-center space-x-2 py-1.5 border border-gray-400 rounded-lg hover:bg-gray-200" aria-label="Forward">
                      <Lucide icon="Share" className="h-6 w-6" />
                      <span>Forward</span>
                    </button>
                  </div>
                ...
                <div className="p-5 flex flex-col-reverse sm:flex-row text-slate-500 border-b border-slate-200/60">
                  <div className="flex items-center mt-3 sm:mt-0 border-t sm:border-0 border-slate-200/60 pt-5 sm:pt-0 -mx-5 sm:mx-0 px-5 sm:px-0">
                    ...
                    <div
                      ...
                      className="w-5 h-5 ml-5 flex items-center justify-center"
                    >
                      <Lucide onClick={handleReload} icon="RefreshCw" className="w-4 h-4" />
                    </div>
                    {currentMessages.some(message => message.isSelected) && (
                      <div className="w-5 h-5 ml-5 flex items-center justify-center">
                        <Lucide onClick={handleDelete} icon="Trash" className="w-4 h-4" />
                      </div>
                    )}
                    ...
                    currentMessages.map((message) => (
                      <div key={message.id} className="intro-y">
                        <div
                          className={classnames({
                            "inbox__item inline-block sm:block text-slate-600 dark:text-slate-500 bg-slate-100 dark:bg-darkmode-400/70 border-b border-slate-200/60 dark:border-darkmode-400": true,
                            "inbox__item--active": message.isSelected || !message.isRead,
                          })}
                        >
                          <div className="flex px-5 py-3">
                            {/* Message item content (unchanged) */}
                            <div className="w-72 flex-none flex items-center mr-5">
                              ...
                              <a
                                ...
                                onClick={() => handleStarMessage(message.id)}
                              >
                                {message.isStared ? <FaStar className="w-4 h-4 text-yellow-500" /> : <FaRegStar className="w-4 h-4" />}
                              </a>

                              <div className="w-6 h-6 flex-none image-fit relative ml-5" onClick={() => handleShowDetails(message)}>
                                <img alt="Sender" className="rounded-full" src={message.senderPhoto} />
                              </div>
                              <div className="inbox__item--sender truncate ml-3" onClick={() => handleShowDetails(message)}>
                                {message.senderName}
                              </div>
                            </div>
                            <div className="w-64 sm:w-auto truncate" onClick={() => handleShowDetails(message)}>
                              <span className="inbox__item--highlight">{message.subject}</span>
                            </div>
                            <div className="inbox__item--time whitespace-nowrap ml-auto pl-10" onClick={() => handleShowDetails(message)}>
                              {message.time}
                            </div>
                          </div>
                        </div>
